...
var GArpMutex sync.Mutex
var GArpBuffers = list.New() //*list.List
...
func (table *NeighboursLookupTable) HandleIPv4ARPPacket(pkt *Packet) error {
	...
	if SwapBytesUint16(arp.Operation) != ARPRequest {
		...
		if SwapBytesUint16(arp.Operation) == ARPReply {
			...
			if (GArpBuffers.Len() > 0) {
				var pktList *list.List
				GArpMutex.Lock()
				pktList = list.New()
				for curPacket := GArpBuffers.Front(); curPacket != nil; curPacket = curPacket.Next() {
					arpPacket := curPacket.Value.(* Packet)
					curIpv4Addr := arpPacket.GetIPv4NoCheck().DstAddr
					if (curIpv4Addr == ipv4) {
						arpPacket.Ether.DAddr = arp.SHA
						// common.LogDebug(common.Debug, "send a packet for", ipv4, ":", entry.MAC)
						arpPacket.SendPacket(table.portIndex)
					} else {
						pktList.PushBack(arpPacket)
					}
				}
				GArpBuffers = pktList
				GArpMutex.Unlock()
...
func (table *NeighboursLookupTable) PrintArpTable() (arptab []string) {
	table.ipv4Table.Range(func(k interface{}, v interface{}) bool {

		entry := v.(neighboursLookupTableEntry)
		ipv4 := k.(types.IPv4Address)
		macaddr := entry.MAC
		arpentry := fmt.Sprint("ARP Entry for", ipv4, ":", macaddr)
		arptab = append(arptab, arpentry)
		return true
	})
	return
}
