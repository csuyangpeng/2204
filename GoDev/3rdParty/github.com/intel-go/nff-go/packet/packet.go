...
var arpBUfMempool *low.Mempool    // arp报文缓存使用的mbuf池
...
func NewArpBufPacket() (*Packet, error) {
	var mb uintptr
	if err := low.AllocateMbuf(&mb, arpBUfMempool); err != nil {
		return nil, err
	}
	pkt := ExtractPacket(mb)
	return pkt, nil
...
func (p *Packet) PacketSetDataLen(data_len uint) {
	low.AppendMbuf(p.CMbuf, data_len)
}

func (p *Packet) FreePacket() {
	buf := uintptr(unsafe.Pointer(p.CMbuf))
	var bufs []uintptr
	bufs = append(bufs, buf)
	low.DirectStop(1, bufs)
}
...
func SetArpBUfMempool(m *low.Mempool) {
	arpBUfMempool = m
}
