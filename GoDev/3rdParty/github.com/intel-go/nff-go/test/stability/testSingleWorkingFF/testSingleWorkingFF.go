...
func setParameters(testScenario uint) (err error) {
	...
	case separate, vseparate:
		if testScenario != generatePart {
			l3Rules, err = packet.GetL3ACLFromTextTable("test-separate-l3rules.config")
		}
	...
	case split, vsplit:
		if testScenario != generatePart {
			l3Rules, err = packet.GetL3ACLFromTextTable("test-split.config")
		}
	...
	case dhandle, vdhandle:
		if testScenario != generatePart {
			l3Rules, err = packet.GetL3ACLFromTextTable("test-handle-l3rules.config")
		}
