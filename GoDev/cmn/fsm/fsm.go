...
func (f *FSM) String() (strbuf string) {
	strbuf += fmt.Sprintf("FSM Info Dump:\n")
	strbuf += fmt.Sprintln("Current: ", f.current)

	for k, v := range f.transitions {
		strbuf += fmt.Sprintln("Event key: ", k, "; Dst: ", v)
	}

	return strbuf
}

func (f *FSM) AddEventDesc(events []EventDesc) {
	// Build transition map and store sets of all events and states.
	for _, e := range events {
		for _, src := range e.Src {
			f.transitions[eKey{e.Name, src}] = e.Dst
...
func (f *FSM) AddCallBack(callbacks map[string]Callback) {

	allEvents := make(map[string]bool)
	allStates := make(map[string]bool)

	for k, v := range f.transitions {
		allStates[k.src] = true
		allStates[v] = true
		allEvents[k.event] = true
	}
...
func (e eKey) String() string {
	return fmt.Sprintf("Event: %s; Src: %s", e.event, e.src)
}
