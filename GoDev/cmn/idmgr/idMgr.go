...
func (p *IDMgr) RegisterIDMgr(idType string, maxID uint32) error {
	p.idMgrRwMu.Lock()
...
func (p *IDMgr) ReserveID(idType string, id uint32) (err error) {
	p.idMgrRwMu.Lock()
	defer p.idMgrRwMu.Unlock()
	...
	err = idMap.reserve(id)

	return
}

func (p *IDMgr) UnReserveID(idType string, id uint32) (err error) {
	p.idMgrRwMu.Lock()
	defer p.idMgrRwMu.Unlock()
	...
	err = idMap.unReserve(id)
	return
}

// BorrowID return a id with id type, will set the flag to busy for the id
func (p *IDMgr) BorrowID(idType string) (id uint32, err error) {
	p.idMgrRwMu.Lock()
	defer p.idMgrRwMu.Unlock()
...
func (p *IDMgr) ReturnID(idType string, id uint32) error {
	p.idMgrRwMu.Lock()
	defer p.idMgrRwMu.Unlock()
...
func (p *IDMgr) GetIDList(idType types.ModuleName) (idList []uint32, err error) {
	p.idMgrRwMu.RLock()
...
func (p *IDMgr) DumpIDList(idType string) {
	...
	idMap, exist := p.idMgrContainer[idType]
	if exist {
		rlogger.Trace(MODULE_ID, rlogger.DEBUG, nil, "IDMap [ %s ] dump info: ", idType)
		idMap.display()
	}
	return
}
