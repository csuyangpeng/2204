...
func GetEndIP(netIp *net.IPNet) (endIP net.IP) {
	//mask : transfer 0 an 1
	onebits, _ := netIp.Mask.Size()
	transOnebitsNum := math.Pow(2, float64(32-onebits)) - 1
	transMask := make([]byte, 4)
	binary.BigEndian.PutUint32(transMask, uint32(transOnebitsNum))
	endIP = make([]byte, 4)
	endIP[0] = transMask[0] | netIp.IP[0]
	endIP[1] = transMask[1] | netIp.IP[1]
	endIP[2] = transMask[2] | netIp.IP[2]
	endIP[3] = transMask[3] | netIp.IP[3] - 1 //reserve .255
	if endIP[3] <= 2 {
		endIP[3] = 2 // end always > start(2)
	}
	return endIP
}

func GetStartIP(netIp *net.IPNet) (startIP net.IP) {
	startIP = make([]byte, 4)
	startIP[0] = netIp.IP[0]
	startIP[1] = netIp.IP[1]
	startIP[2] = netIp.IP[2]
	startIP[3] = 2 //reserve .0 and .1
	return startIP
