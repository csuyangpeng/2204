...
func (p *PduSessionReleaseRequestMsg) Reset() {
	//todo
}

//encode a session release request msg from nas octet stream
func (p *PduSessionReleaseRequestMsg) Encode() ([]byte, error) {
...
// decode a session release request msg from nas octet stream
func (p *PduSessionReleaseRequestMsg) Decode(msgBuf *bytes.Reader) error {
	rlogger.FuncEntry(types.ModCmn, nil)
	// mandatory IEs
	...
	for {
		...
		case nasie.IeiSMCause:
			...
			p.IeFlags.Set(nas.Ieid_SMCause)
		}
	}
	return nil
}
