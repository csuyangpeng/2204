...
PduSessResRelCmdTransferCodec::PduSessResRelCmdTransferCodec() {
  reset();
}

PduSessResRelCmdTransferCodec::~PduSessResRelCmdTransferCodec() {}

void PduSessResRelCmdTransferCodec::setCause(Uint8 cType, Uint8 cValue)
{
	causeType_m = cType;
	causeValue_m = cValue;
}

Uint8 PduSessResRelCmdTransferCodec::getCauseType()
{
    return causeType_m;
}

Uint8 PduSessResRelCmdTransferCodec::getCauseValue()
{
    return causeValue_m;
}

//bool PduSessResRelCmdTransferCodec::encode(NgapOssCtxt* ctxt, MsgBuffer msgBuffer)
MsgBuffer PduSessResRelCmdTransferCodec::encode(NgapOssCtxt *ctxt)
{
	MsgBuffer msgBuffer;
	msgBuffer.length = 0;
...
bool PduSessResRelCmdTransferCodec::decode(NgapOssCtxt *ctxt, MsgBuffer msgBuf)
{
	reset();
...
bool PduSessResRelCmdTransferCodec::reset()
{
 	msgPdu_m.reset_v();
	causeType_m = 0;
	causeValue_m = 0;
...
std::string PduSessResRelCmdTransferCodec::toString()
{

  ostringstream oss;

  oss << "PduSessResRelCmdTransferCodec: type("<<int(causeType_m)<<"," <<int(causeValue_m)<<")";
