...
PduSessResSetupUnSuccTransferCodec::PduSessResSetupUnSuccTransferCodec() {
  reset();
}

PduSessResSetupUnSuccTransferCodec::~PduSessResSetupUnSuccTransferCodec() {}

void PduSessResSetupUnSuccTransferCodec::setCause(Uint8 cType, Uint8 cValue)
{
	causeType_m = cType;
	causeValue_m = cValue;
}

Uint8 PduSessResSetupUnSuccTransferCodec::getCauseType()
{
    return causeType_m;
}

Uint8 PduSessResSetupUnSuccTransferCodec::getCauseValue()
{
    return causeValue_m;
}

//bool PduSessResSetupUnSuccTransferCodec::encode(NgapOssCtxt* ctxt, MsgBuffer msgBuffer)
MsgBuffer PduSessResSetupUnSuccTransferCodec::encode(NgapOssCtxt *ctxt)
{
	MsgBuffer msgBuffer;
	msgBuffer.length = 0;
...
bool PduSessResSetupUnSuccTransferCodec::decode(NgapOssCtxt *ctxt, MsgBuffer msgBuf)
{
	reset();
...
bool PduSessResSetupUnSuccTransferCodec::reset()
{
 	msgPdu_m.reset_v();
	causeType_m = 0;
	causeValue_m = 0;
...
std::string PduSessResSetupUnSuccTransferCodec::toString()
{

  ostringstream oss;

  oss << "PduSessResSetupUnSuccTransferCodec: type("<<(int)causeType_m<<"," <<(int)causeValue_m<<")";
