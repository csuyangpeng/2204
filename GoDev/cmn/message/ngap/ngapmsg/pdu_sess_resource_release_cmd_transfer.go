...
type PduSessResRelCmmdTransfer struct {
	Cause types3gpp.NgapCause

	ctxt codec.NgapOssCtxt
}

func (p PduSessResRelCmmdTransfer) String() string {
	return fmt.Sprintf("%s", p.Cause)
}

// NewCauseTransfer create a cause
func NewPduSessResRelCmmdTransfer() *PduSessResRelCmmdTransfer {
	return &PduSessResRelCmmdTransfer{}
}

// SetOssCodecCtxt set OSS codec context
func (p *PduSessResRelCmmdTransfer) SetOssCodecCtxt(ctxt codec.NgapOssCtxt) {
	p.ctxt = ctxt
}

func (p *PduSessResRelCmmdTransfer) Encode() []byte {
...
func (p *PduSessResRelCmmdTransfer) Decode(msgbuf []byte) error {
