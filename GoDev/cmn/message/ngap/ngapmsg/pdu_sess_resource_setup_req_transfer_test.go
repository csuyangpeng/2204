...
func Test_PduSessResSetupReqTransfer_encode(t *testing.T) {
	ossCtxt := codec.NewOssCtxt()
	ossCtxtPtr := ossCtxt.GetOssCtxtPtr_m()
	defer codec.DeleteOssCtxt(ossCtxt)

	msg := NewPduSessResSetupReqTransfer()
	msg.SetOssCodecCtxt(ossCtxtPtr)

	msg.PduSessType = T.Ipv4
	msg.GtpTunnel.SetTeid(9527)
	msg.GtpTunnel.SetIpAddr(net.ParseIP("10.180.9.242"))

	qosflow := &T.QosFlowSetupReqest{}
	...
	msg.AddQosFlowSetupReq(qosflow)

	qosflow1 := &T.QosFlowSetupReqest{}
	...
	msg.AddQosFlowSetupReq(qosflow1)

	msg.OptFlags.Set(PSRSRT_PduSessAMBR)
	msg.PduSessAMBR.Uplink = 20000
	msg.PduSessAMBR.Downlink = 500000

	//msg.OptFlags.Set(PSRSRT_DataFwdNPssble)
	//msg.DataFwdNotPssble = T.DataFwdNotPossible

	//msg.OptFlags.Set(PSRSRT_SecInd)
	//msg.SecIndication.IntPrctInd = T.IntPrctIndRequired
	//msg.SecIndication.ConfdPrctInd = T.ConfdPrctIndNotNeeded
...
func Test_PduSessResSetupReqTransfer_decode(t *testing.T) {
