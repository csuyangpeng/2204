...
// CheckHealth for redis connection
func (c *Cacher) CheckHealth() (err error) {
	conn := c.pool.Get()
	defer conn.Close()

	if _, err := conn.Do("PING"); err != nil {
		err = fmt.Errorf("failed to connect to redis ser, err=%v", err)
	}
	return
}
...
Redis SET
**/
func (c *Cacher) SetAdd(key string, member string) (reply interface{}, err error) {
	return c.Do("SADD", c.getKey(key), member)
}

func (c *Cacher) SetMembers(key string) (reply []interface{}, err error) {
	values, err := redis.Values(c.Do("SMEMBERS", c.getKey(key)))
	if err != nil {
		return nil, err
	}
	return values, err
...
func (c *Cacher) SetRem(key string, member string) (reply interface{}, err error) {
