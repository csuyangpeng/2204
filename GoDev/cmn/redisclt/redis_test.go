...
func TestCacher_SetAdd(t *testing.T) {
	var err error
	c := getCacher()
	_, err = c.SetAdd("test_sc_insts", fmt.Sprintf("%d", 1))
	NoError(t, err)

	_, err = c.SetAdd("test_sc_insts", fmt.Sprintf("%d", 2))
	NoError(t, err)

	_, err = c.SetAdd("test_sc_insts", fmt.Sprintf("%d", 3))
	NoError(t, err)
	_, err = c.SetAdd("test_sc_insts", fmt.Sprintf("%d", 4))
	NoError(t, err)

	dat, err := c.SetMembers("test_sc_insts")
	fmt.Println(dat)
	NoError(t, err)
}
